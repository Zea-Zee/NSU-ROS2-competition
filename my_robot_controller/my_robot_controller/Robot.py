...
class Robot(Node):
    def __init__(self, camera=None, odom=None, twist=None):
        ...
        self.create_subscription(Image, '/color/image', self._camera_callback, 10)
    ...
    def just_follow(self, robot: Robot):
        ...
        if num_labels > 1:
            ...
            for i in range(1, num_labels):
                cv2.circle(dst, (int(centroids[i][0]), int(centroids[i][1])), 2, (0, 255, 0), 2)
                centroid = centroids[i]

                # Расчёт расстояний до предыдущих точек
                distance1 = abs(centroid[0] - (self.prevpt1[0] if self.prevpt1 else 0))
                distance2 = abs(centroid[0] - (self.prevpt2[0] if self.prevpt2 else 0))
            ...
            cpt1 = tuple(centroids[min_idx1]) if min_dist1 < 100 else self.prevpt1
            cpt2 = tuple(centroids[min_idx2]) if min_dist2 < 100 else self.prevpt2
        else:
