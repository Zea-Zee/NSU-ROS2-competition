...
# Инициализируем колораму для цветого логгинга
colorama_init()
    ...
    def run_detector(self) -> None:
        ...
        try:
            if self.cv_image is not None:
                ...
                boxes, self.yolo_image, duration = self.detector.process_image(
                    self.cv_image)
            ...
            self.get_logger().error(
                f"Robot.run_detector exception: {Fore.RED}{e}{Fore.RESET}")
    ...
    def check_for_state_transition(self, boxes):
        ...
        for box in boxes:
            ...
            if box['label'] == 'tunnel_sign' and box['area'] > 10000.0 and box['conf'] > 0.85 and self.state_machine.set_next_state('tunnel'):
                self.get_logger().info("States was updated to tunnel with")
                self.get_logger().info(
                    f"{box['conf']} conf and {box['area']} area")
